...
int run_cascade(void *cascade, float *o, int r, int c, int s, void *vppixels, int nrows, int ncols, int ldim)
{
	...
	*o = 0.0f;
	...
	for (i = 0; i < ntrees; ++i) // for each tree layer
	{
		//
		tcodes = ptree - 4;
		lut = (float *)(ptree + ((1 << tdepth) - 1) * sizeof(int32_t));
		thr = *(float *)(ptree + ((1 << tdepth) - 1) * sizeof(int32_t) + (1 << tdepth) * sizeof(float)); // tree threshold
		...
		for (j = 0; j < tdepth; ++j)
			idx = 2 * idx +
				  (pixels[(r + tcodes[4 * idx + 0] * s) / 256 * ldim + (c + tcodes[4 * idx + 1] * s) / 256] <=
				   pixels[(r + tcodes[4 * idx + 2] * s) / 256 * ldim + (c + tcodes[4 * idx + 3] * s) / 256]);

		*o = *o + lut[idx - (1 << tdepth)];

		//
		if (*o <= thr) // less than threshold

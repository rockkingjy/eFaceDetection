...
int nimages = 0;		 // #(images)
uint8_t *ppixels[MAX_N]; // image pixels
int pdims[MAX_N][2];	 // (nrows, ncols) for the image
int nobjects = 0;		 // #(bbox in total)
int objects[MAX_N][4];   // bbox (r, c, s, index of image this bbox belongs to)
int contents[MAX_N][2];  // (i, j), i is the start pointer and j-1 is the ending pointer of the bbox

// trees
#define MAX_N_TREES 4096
int32_t version = 3;
int tdepth = 6;					   // depth of trees, 6
int ntrees = 0;					   // #trees
int8_t bbox[4];					   // (r_min, r_max, c_min, c_max), {-127, +127, -127, +127}
int32_t tcodes[MAX_N_TREES][1024]; // non-leaf node feature pairs, (2+2)x2^8=1024
float luts[MAX_N_TREES][1024];	   // leaf node outputs
float thresholds[MAX_N_TREES];	   // thresholds of trees

// seleceted images
static int rs[2 * MAX_N];	  // row of bbox
static int cs[2 * MAX_N];	  // col of bbox
static int ss[2 * MAX_N];	  // scale of bbox
static int iinds[2 * MAX_N];   // index of image this bbox belongs to
static float tvals[2 * MAX_N]; // true values for the images
static float os[2 * MAX_N];	// Output of the tree
...
#ifdef __GNUC__
#include <time.h>
float getticks() // get the time now
{
	struct timespec ts;
...
/*
	Multiply-with-carry pseudorandom number generator(PRNG)
*/
...
/*
	regression trees
*/
// iind: index of image; (r,c,s): ; tcode: node feature pairs.
int bintest(int32_t tcode, int r, int c, int s, int iind)
{
	...
	r2 = (256 * r + p[2] * s) / 256;
	c2 = (256 * c + p[3] * s) / 256;
	// keep the two points in the image
...
// calculate WMSE
float get_split_error(int32_t tcode, float tvals[], int rs[], int cs[], int ss[],
					  int iinds[], double ws[], int inds[], int indsnum)
{
	...
	{
		if (bintest(tcode, rs[inds[i]], cs[inds[i]], ss[inds[i]], iinds[inds[i]])) // C1
		{
			wsum1 += ws[inds[i]]; // sum of weights
			wtvalsum1 += ws[inds[i]] * tvals[inds[i]]; // sum of weighted true value
...
int split_training_data(int32_t tcode, float tvals[], int rs[], int cs[], int ss[],
...
// grow_subtree recursively.
int grow_subtree(int32_t tcodes[], float lut[], int nodeidx, int d, int maxd,
				 float tvals[], int rs[], int cs[], int ss[], int iinds[],
				 double ws[], int inds[], int ninds, int8_t *bbox)
{
	int i;
	int32_t tmptcodes[NRANDS];
	float es[NRANDS], e;
	int n0;

	if (d == maxd) // if reach the deepest layer
	{
		int lutidx = nodeidx - ((1 << maxd) - 1); // nodeidx - #(non-leafs)
		double tvalaccum = 0.0;
		double wsum = 0.0;
		// compute output: a simple average
		for (i = 0; i < ninds; ++i)
		{
			tvalaccum += ws[inds[i]] * tvals[inds[i]]; // weighted sum of true values
			wsum += ws[inds[i]]; // sum of weights
		}

		if (wsum == 0.0)
			lut[lutidx] = 0.0f;
		else
			lut[lutidx] = (float)(tvalaccum / wsum); // leaf node output = average of weighted sum

		return 1;
	}
	else if (ninds <= 1) // if not reach the deepest layer & #images <= 1, no need to split C0 and C1.
	{
		tcodes[nodeidx] = 0;
	...
	// generate binary test codes randomly
	for (i = 0; i < NRANDS; ++i)
		tmptcodes[i] = get_random_tcode(bbox);

	// calculate the WMSE for each pair
#pragma omp parallel for
	for (i = 0; i < NRANDS; ++i)
		es[i] = get_split_error(tmptcodes[i], tvals, rs, cs, ss, iinds, ws, inds, ninds);

	// find the feature pair with the least error
	e = es[0];
...
int grow_rtree(int32_t tcodes[], float lut[], int d,
			   float tvals[], int rs[], int cs[], int ss[],
			   int iinds[], double ws[], int n, int8_t *bbox)
{
...
float get_tree_output(int i, int r, int c, int s, int iind)
{
	...
	{
		// if bintest()==1, right branch; else left branch.
		idx = 2 * idx + bintest(tcodes[i][idx - 1], r, c, s, iind);
	}
	return luts[i][idx - (1 << tdepth)];
}
// check whether (r,c,s) is a object or not.
...
float learn_new_stage(float mintpr, float maxfpr, int maxntrees,
					  float tvals[], int rs[], int cs[], int ss[],
					  int iinds[], float os[], int np, int nn)
{
	...
	while (ntrees < maxntrees && fpr > maxfpr)
	{
		...
		for (i = 0; i < np + nn; ++i)
		{
			float o = get_tree_output(ntrees - 1, rs[i], cs[i], ss[i], iinds[i]);
			os[i] += o;
		}

		// get threshold according to the min TPR...
		threshold = 5.0f;
		do
		{
			threshold -= 0.001f;
...
int find_objects(
	float rcsq[], int maxndetections, int iind,
	float scalefactor, float stridefactor, float minsize, float maxsize)
{
	float s = minsize;
	int ndetections = 0;
	int nrows = pdims[iind][0];
	int ncols = pdims[iind][1];
	...
	while (s <= maxsize)
	{
		...
		for (r = s / 2 + 1; r <= nrows - s / 2 - 1; r += dr)
			...
			{
				float q;
				int t = classify_region(&q, r, c, s, iind);
				if (1 == t)
				{
...
#pragma omp parallel for
	for (int i = 0; i < nimages; ++i) // for each training image
	{
		int thid = omp_get_thread_num();
		float dets[4 * MAXNDETS];
		int ndets = find_objects(dets, MAXNDETS, i, 1.1f, 0.1f, 24, 1000);//get potential detections
		...
		{
			int assigned = 0;
			for (int k = contents[i][0]; k < contents[i][1]; ++k)
			{
				float overlap = get_overlap(dets[4 * j + 0], dets[4 * j + 1], dets[4 * j + 2], objects[k][0], objects[k][1], objects[k][2]);
				if (overlap > 0.6f && *np < MAX_N / 2 && n < MAX_N) // if overlap > 0.6, then regard it as positive
#pragma omp critical
				{
					// true positive
					rs[n] = dets[4 * j + 0];
...
float sample_training_data(float tvals[], int rs[], int cs[], int ss[], int iinds[], float os[], int *np, int *nn)
{
	...
	*/
	for (int i = 0; i < nobjects; ++i)
	{
		...
		s = objects[i][2];
		iind = objects[i][3];
		if (classify_region(&os[n], r, c, s, iind) == 1) // check whether it's positive 
		{
			rs[n] = r;
			cs[n] = c;
			ss[n] = s;
	...
	*/
	...
	nw = 0;  // #(all the negative images)
	*nn = 0; // #(selected negative images)
	stop = 0;
		...
		while (!stop)
		{
			...
			*/
			...
			if (classify_region(&o, r, c, s, iind) == 1)
			{
				...
				// if > 0.5, then discard.
				int ok = 1;
				for (int i = contents[iind][0]; i < contents[iind][1]; ++i)
					if (get_overlap(r, c, s, objects[i][0], objects[i][1], objects[i][2]) > 0.5f)
						ok = 0;

				if (ok) // if <= 0.5, then select it.
#pragma omp critical
				{
					...
					else // if selected negative >= selected positive images
					{
						stop = 1;
					}
				}
...
#pragma omp atomic
	...
	*/
	etpr = *np / (float)(nobjects);   // #(selected positive images) / #(all the positive images)
	efpr = (float)(*nn / (double)nw); // #(selected negative images) / #(all the negative images)
...
/*
	...
	_tdepth=6; stagetpr=0.98; stagefpr=0.4; maxntreesperstage=16;
*/
int learn_a_cascade(char *savepath, int8_t bb[], int _tdepth,
					float stagetpr, float stagefpr, int maxntreesperstage)
{
	int np, nn, i;
	FILE *f = 0;
	bbox[0] = bb[0];
	...
	// sample and train untill FPR < 0.01f.
	while (1)
	{
	...
	char save[6][9] = {"1.weight", "2.weight", "3.weight", "4.weight", "5.weight", "6.weight"};
	float subsf = 0.01f;
	for (i = 0; i < 5; ++i)
	{
		// training
...
const char *howto()
{
	return "./picolrn <trdata> <cascade-write-path>\n";
}
